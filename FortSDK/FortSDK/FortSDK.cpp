...
SDK::uint32 SDK::Offsets::Members::UClass_DefaultObj = -1;
...
template <typename T>
static uint32_t FindOffset(void* obj, T target, int start = 0) {
        auto p = static_cast<uint8_t*>(obj);
        for (int i = start; i < 0x2000; i++)
                if (*reinterpret_cast<T*>(p + i) == target) return i;
...
bool SDK::Init() {
    ...
    */
        ...
        */
        ...
        memLibrary->SwitchPattern(
            "48 89 5C 24 08 48 89 74 24 10 57 48 83 EC ? 48 8B F1 41 8B D8 48 8B 0D ? ? ? ?" );
        if (memLibrary->GetStatus() != 0)
        {
    ...
    auto sz = VEnd - VStart;
    wchar_t* s = (wchar_t*)malloc((sz * 2) + 2);
    __movsb((PBYTE)s, (const PBYTE)VStart, sz * 2);
    s[sz] = 0;
    wchar_t* e;
    auto ret = wcstod(s, &e);
    free(s);
    return ret;
        ...
        if (FNVer >= 16) {
                Offsets::Members::UFunction__Exec = uint32_t(0xD8);
        }
        ...
        */
        ...
        if (!GetEngineVersionFN) {
                logLibrary->Log_Internal(
                    "Failed to find GetEngineVersion function. "
                    "Please alert NotTacs on discord.");
                return false;
        }
        ...
        */
        ...
        if ( memLibrary->GetStatus() != 0 ) {
            ...
            Offsets::UObject__ProcessEvent = memLibrary->GetAddress();
                ...
                auto Addr = Memcury::Scanner::FindStringRef(L"UMeshNetworkComponent::ProcessEvent: Invalid mesh network node type: %s", true, 0, Engine_Version >= 5.00 && Fortnite_Version.Season() < 24);
                auto Wow = Memcury::Scanner(FindBytes(Addr, { 0xE8 }, 2000, 0, false, Engine_Version < 5.00 ? 1 : 3)).RelativeOffset(1).Get();
                if (Wow) {
                        UE_LOG( LogFortSDK, Log, "ProcessEvent: 0x%x", Wow - uint64(GetModuleHandle( 0 )) );
                        Offsets::UObject__ProcessEvent = Wow;
                } else {
                        logLibrary->Log_Internal(
    ...
    "NotTacs on discord." );
        ...
        if (!Offsets::UObject__ProcessEvent) {
                auto Addr = Memcury::Scanner::FindStringRef(L"UMeshNetworkComponent::ProcessEvent: Invalid mesh network node type: %s", true, 0, Engine_Version >= 5.00 && Fortnite_Version.Season() < 24);
                auto Wow = Memcury::Scanner(FindBytes(Addr, { 0xE8 }, 2000, 0, false, Engine_Version < 5.00 ? 1 : 3)).RelativeOffset(1).Get();
                if (Wow) {
                        Offsets::UObject__ProcessEvent = Wow;
        ...
        Offsets::Members::UClass_DefaultObj = FindOffset(StaticClassImpl( "Actor" ), GUObjectArray.FindObject( "Default__Actor" ));

        logLibrary->Log_Internal( "UClass::DefaultObject: %p",
                                  Offsets::Members::UClass_DefaultObj );
